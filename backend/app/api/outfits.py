...
async def get_sustainability_insights(user_id: str):
    """Get sustainability insights and wardrobe usage analytics"""

    # Mock wardrobe data - same as above, in real app fetch from database
    mock_wardrobe = [
        ClothingItem(
            id="item1", name="Blue Jeans", type="jeans", color="blue",
            tags=["casual", "everyday"], usage_count=8, last_worn=None,
        ...
        ClothingItem(
            id="item4", name="Denim Jacket", type="jacket", color="blue",
            tags=["casual", "layering"], usage_count=2, last_worn=None,
            is_formal=False, is_seasonal=False
        ),
        ClothingItem(
            id="item5", name="Black Blazer", type="blazer", color="black",
        ...
        ClothingItem(
            id="item7", name="Black Dress Pants", type="pants", color="black",
            tags=["formal", "professional"], usage_count=1, last_worn=None,
            is_formal=True, is_seasonal=False
        ),
        ClothingItem(
            id="item8", name="Red Dress", type="dress", color="red",
            tags=["formal", "party"], usage_count=1, last_worn=None,
            is_formal=True, is_seasonal=False
        )
    ]

    # Get sustainability analysis
    insights = recommendation_engine.analyze_wardrobe_usage(mock_wardrobe)
    ...
    # Penalty for too many rarely worn items
    rarely_worn_penalty = (rarely_worn_count / total_items) * 50 if total_items > 0 else 0

    sustainability_score = max(0, usage_efficiency - rarely_worn_penalty)
    ...
    return {
        ...
        "recommendations": [
            ...
            "Before buying new items, check if you can create similar looks with what you have",
            "Donate items you haven't worn in over a year"
        ]
    }